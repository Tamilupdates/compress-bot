...
async def set_timer(event, list1, list2):
    ...
    await event.client.send_message(event.chat_id, 'You can start a new process.')
...
async def check_timer(event, list1, list2):
    if f'{event.sender_id}' in list1:
        ...
        present = time.time()
        return False, f"You have to wait {0-round(present-float(last))} seconds more to start a new process!"
    else:
        return True, None
